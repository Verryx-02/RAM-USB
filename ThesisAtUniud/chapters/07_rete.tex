...
\section{Politiche di rete e confini di raggiungibilità}

Le zone di fiducia della \autoref{fig:trust-zones} descrivono chi può contattare chi. I confini di raggiungibilità
...
\section{Ciclo di vita dei permessi di accesso}
...
\begin{lstlisting}[language=sql, caption={Schema della tabella dei permessi di accesso, da \texttt{store.go}
...
Un processo periodico, ogni cinque minuti, cerca le scadenze passate e revoca i permessi scaduti
...
Se nel frattempo un login ha rinnovato il permesso, la scadenza in tabella non coincide più, la cancellazione
...
%% \clearpage qui, non subito dopo fig:acl-grant: forzare il taglio pagina prima dell'ultima
%% sezione del capitolo, invece che a metà della precedente, evita di lasciare vuota la pagina
%% della figura (§7.3 la riempie normalmente) e sposta lo spazio residuo sull'ultima pagina del
%% capitolo, dove è normale trovarne. Serve comunque un \clearpage esplicito qui vicino alla
%% fine del capitolo: senza, la figura resta in sospeso fino a dopo l'apertura del capitolo 8,
%% causando due pagine bianche di troppo al cambio capitolo (visto in build/no-dedica, pp. 37-38).
\clearpage

\section{CG-NAT e la necessità di un endpoint pubblico dedicato}

Visto che gli indirizzi IPv4 sono ormai terminati, gli Internet Service Providers tendono ad assegnare ai router 
