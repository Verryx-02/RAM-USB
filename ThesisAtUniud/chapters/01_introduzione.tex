...
Inexpensive Disks) \cite{patterson-gibson-katz-raid-1988}, che distribuisce lo stesso dato su più dischi 
economici per ottenere un'affidabilità paragonabile a quella di un singolo disco più costoso.
...
email e password che, in quanto dati sensibili, richiedono un livello di protezione elevato. 
...
Il fatto che il server possieda le credenziali espone, però, tutti gli utenti al rischio che vengano
...
\begin{itemize}
    ...
    \item \textbf{resistenza alle collisioni:} è computazionalmente intrattabile trovare una qualsiasi coppia
...
Tra le diverse cause di perdita di dati, il progetto si concentra su quella derivante dall'attacco di un 
malintenzionato, adottando i principi di zero-trust e defense-in-depth per garantire la 
sicurezza dei dati anche in caso di compromissione di un componente qualsiasi del sistema.
...
supervisione umana. 
La progettazione, la struttura del codice, i confini dei moduli, le firme, il comportamento delle singole 
...
Le numerose strategie di mitigazione degli errori implementativi e allucinazioni dei due modelli, nonché la
gestione dei multipli agenti utilizzati non sono oggetto di questa tesi.

\section{Struttura della tesi}

Il \autoref{chap:principi-fondanti} approfondisce i due principi di sicurezza su cui RAM-USB è costruito,
zero-trust e defense-in-depth.
