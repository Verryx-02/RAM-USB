...
\title{RAM-USB: Progettazione, implementazione e distribuzione di un servizio di backup multi-utente secondo i principi zero-trust e defense-in-depth \\ }
...
%% xurl: gli URL lunghi nella bibliografia (i permalink GitHub in particolare) vanno a capo solo dopo
%% "/", lasciando righe cortissime seguite da una lunghissima. xurl permette di spezzarli a qualunque
%% carattere, distribuendo il testo in modo più uniforme.
\usepackage{xurl}
%% Di default \UrlFont è solo \ttfamily, senza una dimensione propria, quindi eredita quella del
%% contesto: la lascio così (non forzo una dimensione qui) in modo che le dimensioni scelte entro
%% ogni singola voce del .bib (\small per le fonti esterne, \footnotesize per le autocitazioni,
%% così da restare meno invadenti) restino effettive invece di essere sovrascritte qui.
\usepackage{float}
...
%% dirtree: disegna una struttura di directory come albero, con commenti allineati a destra.
\usepackage{dirtree}
...
%% Niente \cleardoublepage qui: \listoftables apre già da sé un \chapter* su pagina dispari
%% (stesso meccanismo automatico di Ringraziamenti/Sommario/Indice sopra) — aggiungerne uno
%% esplicito raddoppierebbe il salto pagina e sfaserebbe la voce nell'indice di una pagina.
\phantomsection
...
%% Elenco degli acronimi: resta qui, prima di \mainmatter, così ha numerazione romana come le
%% altre liste sopra e non ruba la pagina 1 (araba) al primo capitolo.
\input{chapters/00_acronimi}
...
%% \bibliography da solo non si aggiunge all'indice (a differenza di \chapter, che lo fa in automatico):
%% serve un \addcontentsline esplicito; \phantomsection garantisce che hyperref punti all'ancora giusta.
%% \cleardoublepage va PRIMA di \addcontentsline, non dopo: \bibliography inserisce il salto pagina
%% al suo interno, quindi chiamare prima addcontentsline registrerebbe ancora la pagina vecchia.
%% openright (default di book, twoside, nessun openany in thud.cls) vuole pagina dispari, come ogni
%% altro \chapter della tesi.
\cleardoublepage
\phantomsection
