...
// unit tests to run with no external dependency and no Docker, so this test
// is skipped whenever the variable is unset, instead of being a hard
...
func TestNewAndApply_CreatesUsersTableFromEmptyDatabase(t *testing.T) {
	...
	if databaseURL == "" {
		t.Skipf("%s not set; skipping the real-Postgres schema test (see docs/Test_Plan.md section 4: unit tests run with no external dependency, no Docker). Start deployments/compose/postgres.yml's database-vault-postgres service and set this variable to run it.", databaseURLEnvVar)
	}
