...
// deployments/compose/postgres.yml). docs/Test_Plan.md section 4 requires
// unit tests to run with no external dependency and no Docker, so — unlike
// every other DV-F-08 test in this package, which uses a hand-written fake
// — this test is skipped whenever the variable is unset, instead of being a
// hard dependency of `go test ./...`.
...
func TestSaveUser_Postgres(t *testing.T) {
	...
	if databaseURL == "" {
		t.Skipf("%s not set; skipping the real-Postgres DV-F-08 test (see docs/Test_Plan.md section 4: unit tests run with no external dependency, no Docker). Start deployments/compose/postgres.yml's database-vault-postgres service and set this variable to run it.", databaseURLEnvVar)
	}
