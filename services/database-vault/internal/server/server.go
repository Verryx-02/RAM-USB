...
// mTLS client connecting to Database-Vault. Security-Switch is the only
// component authorized to call Database-Vault directly.
const AllowedClientOrganization = "SecuritySwitch"

// NewTLSConfig returns the mTLS server configuration Database-Vault uses to
// satisfy DV-F-01: a connection completes its handshake only if the peer
...
// network, is enforced by Network-Manager's ACL rules (NM-F-02), not here.
