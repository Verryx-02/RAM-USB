	...
	if err != nil {
		if errors.Is(tokenErr, pki.ErrBootstrapTokenMissing) {
			return nil, fmt.Errorf(
				"bootstrap mqtt client identity from certificate-authority: neither a persisted identity under %s nor a token in %s is available (the token is only needed on this service's very first start): %w",
