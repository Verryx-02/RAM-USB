...
- The real Proxmox LXC guest's own `/dev/net/tun` + `NET_ADMIN`/`NET_RAW`
  privilege grant - only the Docker-container-level equivalent has been
  exercised so far (the dev Compose stack), not a real LXC guest config.
