...
# below has to live in this separate file instead: "with-contenv /bin/sh
# /etc/security-switch/tailscale-up.sh" is what that up file execs, per
# with-contenv's own "with-contenv prog args..." contract) performs the
...
# security-switch longrun (dependencies.d/tailscale-up) waits for this to
# finish before starting, since it binds exclusively to that address
# (NET-F-01).
#
...
# RAM_USB_TAILSCALE_CONTROL_URL/RAM_USB_SECURITY_SWITCH_TAILSCALE_AUTHKEY/
# RAM_USB_SECURITY_SWITCH_MESH_HOSTNAME pushed back into its environment
# (S6_KEEP_ENV stays at its default 0, same convention already documented
# in ../s6-overlay/s6-rc.d/security-switch/run for the security-switch
# longrun).
set -eu
...
# reads and caches the OS certificate pool at its own process start - see
# trust-mesh-ca.sh's own doc comment for the full reasoning.
...
tailscale up \
	...
	--authkey="${RAM_USB_SECURITY_SWITCH_TAILSCALE_AUTHKEY:?RAM_USB_SECURITY_SWITCH_TAILSCALE_AUTHKEY is not set}" \
	--hostname="${RAM_USB_SECURITY_SWITCH_MESH_HOSTNAME:-security-switch}" \
	--reset
...
# oneshot (security-switch) via s6-overlay's container environment
# directory - with-contenv (used by that longrun's own run script) re-reads
# this directory fresh on every process start, so a file written here after
# container init is picked up exactly like a docker-compose-supplied
# variable would be.
mkdir -p /var/run/s6/container_environment
printf '%s' "$TS_IP" >/var/run/s6/container_environment/RAM_USB_SECURITY_SWITCH_TAILSCALE_IP
