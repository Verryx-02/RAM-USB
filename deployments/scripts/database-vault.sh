#!/bin/bash
# Shell 6 - Database-Vault, now co-located with its own Postgres in one
...
docker exec headscale headscale users create database-vault 2>/dev/null || true
DV_ID=$(docker exec headscale headscale users list -o json | jq -r '.[] | select(.name=="database-vault") | .id')
...
export RAM_USB_DATABASE_VAULT_TAILSCALE_AUTHKEY=$(docker exec headscale headscale preauthkeys create --user "$DV_ID" --expiration 30m --tags tag:database-vault)
