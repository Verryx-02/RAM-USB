#!/bin/bash
# Shell 8 - Security-Switch (+ mesh). Requires shells 1, 2.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/../.."

docker network create ramusb-net 2>/dev/null || true

docker exec headscale headscale users create security-switch 2>/dev/null || true
SS_ID=$(docker exec headscale headscale users list -o json | jq -r '.[] | select(.name=="security-switch") | .id')
...
export RAM_USB_SECURITY_SWITCH_TAILSCALE_AUTHKEY=$(docker exec headscale headscale preauthkeys create --user "$SS_ID" --expiration 30m --tags tag:security-switch)
