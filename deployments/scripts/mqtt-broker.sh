#!/bin/bash
# Shell 3 - MQTT-Broker + its Tailscale mesh sidecar. Requires shells 1, 2.
...
docker exec network-manager headscale users create mqtt-broker 2>/dev/null || true
MQTT_ID=$(docker exec network-manager headscale users list -o json | jq -r '.[] | select(.name=="mqtt-broker") | .id')
export RAM_USB_TAILSCALE_CONTROL_URL="https://network-manager:8080"
export RAM_USB_MQTT_BROKER_TAILSCALE_AUTHKEY=$(docker exec network-manager headscale preauthkeys create --user "$MQTT_ID" --expiration 30m --tags tag:mqtt-broker)
