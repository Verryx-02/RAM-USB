...
|CL-F-06|Following a user command, must invoke `restic backup` against Storage-Service via SFTP **on TCP port 2222** (ST-F-03), authenticating with the SSH private key generated in CL-F-01|[Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/user-client/internal/restic/restic.go#L118-L130)|
|CL-F-07|Following a user command, must invoke `restic restore` against Storage-Service via SFTP **on TCP port 2222** (ST-F-03), using the same authentication method as CL-F-06|[Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/user-client/internal/restic/restic.go#L132-L147)|
...
|CL-F-10|Must take Entry-Hub's login endpoint from a configuration value **separate** from its registration endpoint, and must use for login a hostname covered by the certificate Entry-Hub presents|Added 2026-08-23. EH-F-01/EH-F-02 and EH-F-03 are served on two different listeners at two different addresses (public vs mesh-only), so one base URL cannot address both: registration against the mesh address is unreachable before CL-F-04's join, and login against the public address is answered with 404. Because EH-F-03 presents the *same* publicly-signed certificate on both listeners, the login value's hostname must be the public one that certificate covers, not a mesh IP or MagicDNS name — NM-F-19 is what makes that hostname resolve to the mesh address for mesh members.|
|CL-F-11|Must pin Storage-Service's SSH host key: it must record the host key received in the registration response (ST-F-16) before the first backup, present it to `ssh` as a known host, and abort the connection if the key presented at connection time differs|Added 2026-08-23. Before this the Client passed no host-key policy at all, so the first backup on any machine failed with `Host key verification failed.` Trust-on-first-use would have been the smaller change, and is arguably sufficient inside a mesh whose transport already authenticates every peer by WireGuard public key — but the key is available over an authenticated mTLS chain at registration time, so pinning it costs nothing in trust and keeps the defence-in-depth stance the rest of the system takes (RNF-SEC-02/03).|
...
|ST-F-03|Access must occur exclusively via SFTP on **TCP port 2222**, authenticated with the user's registered SSH public key|The port was left unstated until 2026-08-23 and is named here because it is the one value the two sides must agree on: `sshd` binds 2222 exclusively (`CAP_NET_BIND_SERVICE` is deliberately outside the container's capability set, so port 22 is not bindable at all) while the Client dialled the default 22, which made UC-03/UC-04 fail with `Connection refused` on the first real end-to-end run. See CL-F-06/CL-F-07. [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/deployments/docker/storage-service/sshd_config#L43-L46)|
...
|ST-F-16|Must return its SSH host public key in the response to Database-Vault's POSIX-user creation request (ST-F-10), so that it can be relayed back to the Client and pinned there (CL-F-11)|Added 2026-08-23. The key travels the same already-authenticated mTLS chain the POSIX username already travels (Storage-Service → Database-Vault → Security-Switch → Entry-Hub → Client), so no new trust assumption is introduced. It is a public key: relaying it leaks nothing.|
...
|**ID**|**Requirement**|**Notes**|
|---|---|---|
|NM-F-01|Must ensure that only Entry-Hub can contact Security-Switch|Narrowed 2026-08-23 from "Entry-Hub, Database-Vault, Network-Manager, and Certificate-Authority" after measuring the real call graph on the Proxmox deployment: Entry-Hub is the only component that ever dials Security-Switch (`RAM_USB_SECURITY_SWITCH_URL` exists solely in `services/entry-hub`), and Security-Switch's own mTLS gate accepts `organization=EntryHub` only, so the other three could never authenticate even if they connected. The three were originally listed because the requirement was written as the union of "who calls Security-Switch" and "who Security-Switch calls", on the assumption that reply traffic needs a rule in the opposite direction; it does not — Tailscale's packet filter matches ACL rules only against a TCP SYN and accepts every subsequent packet of an established session, confirmed live (Entry-Hub cannot open a connection to a user node yet serves that node's requests normally). [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/services/network-manager/internal/headscale/policy.go#L182-L187)|
|NM-F-02|Must ensure that only Security-Switch and Storage-Service can contact Database-Vault|Certificate-Authority dropped 2026-08-23 for the reason given in NM-F-01: step-ca never initiates a connection to any service. Security-Switch (register/login) and Storage-Service (ST-F-11's `AuthorizedKeysCommand` public-key lookup) are both real initiators and stay. [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/services/network-manager/internal/headscale/policy.go#L188-L193)|
|NM-F-03|Must ensure that only Security-Switch can contact Network-Manager|Certificate-Authority dropped 2026-08-23 for the reason given in NM-F-01. Note Network-Manager's own outbound admin traffic to Headscale is unaffected: it crosses the public network, not the mesh (NM-F-12/NM-F-14). [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/services/network-manager/internal/server/server.go#L1-L36) / [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/services/network-manager/internal/headscale/policy.go#L194-L206)|
|NM-F-04|Must ensure that all internal components of the network, except Users, can contact the Certificate-Authority over the mesh network|The "and be contacted by" half was dropped 2026-08-23: it produced a second ACL rule granting the Certificate-Authority reachability toward all eight internal components, and step-ca never initiates a connection to any of them. Its only outbound traffic is the metrics sidecar's MQTT publish (CA-F-03), which the broker's own rule already covers. Reply traffic on connections the services themselves open needs no rule in this direction — see NM-F-01. [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/services/network-manager/internal/headscale/policy.go#L207-L224)|
...
|NM-F-19|Must configure MagicDNS with a record resolving Entry-Hub's public hostname to Entry-Hub's **mesh** address, so that a mesh member contacting the login listener (EH-F-03) uses the hostname Entry-Hub's publicly-signed certificate covers|Added 2026-08-23. Without it CL-F-10 has no working value to be configured with: the login listener is reachable only at a mesh address, but presents a certificate issued for the public hostname, so connecting by mesh IP or by MagicDNS short name fails hostname verification. Headscale's `dns.extra_records` provides this.|
|NM-F-20|Must ensure that only the services that publish metrics (Entry-Hub, Security-Switch, Database-Vault, Storage-Service, Network-Manager, Certificate-Authority) and Metrics-Collector can contact MQTT-Broker|Added 2026-08-23 (KI-119). The corresponding ACL rule already existed in `policy.go`, written on the implementer's own judgment because no requirement covered observability reachability at all; `policy.go`'s own comment flagged it as needing confirmation. Stating it here matters more than it looks: once any policy exists Headscale is default-deny, so an unstated rule that someone later removes as "unused" would take the whole metrics pipeline (MT-F-01..04, CA-F-03, DV-F-16, EH-F-10, NM-F-17, SS-F-07, ST-F-12) down silently, with no error anywhere.|
|NM-F-21|Must ensure that only Grafana can contact Metrics-Collector|Added 2026-08-23 (KI-119), same reasoning as NM-F-20. TimescaleDB is co-located inside Metrics-Collector's own container, so Grafana's outbound query connection (UC-05, RU-10) reaches it under this rule rather than a separate one.|
...
|CA-F-04|Must accept a single-use bootstrap token, distributed out-of-band to each service, for issuing the initial certificate; subsequent renewals must occur via the current mTLS certificate, not via the token. Each service must therefore **persist the certificate and key it is issued**, so that the token is consumed exactly once in the service's lifetime and a process or container restart renews from the stored certificate instead of re-presenting the token|The persistence clause was made explicit 2026-08-23 after finding the implementation violates this requirement as originally worded: `pkg/pki` holds the issued identity in memory only, so every process start re-runs the bootstrap exchange, and the second start of any container presents an already-spent token and is refused with `The request lacked necessary authorization to be completed`. Consequences measured live: `restart: unless-stopped` cannot recover a crashed service, and a host reboot needs an operator to mint one token per service by hand. See KI-126. [Code](https://github.com/Verryx-02/RAM-USB/blob/7c1e991f953a4c686e09f8142d0b3c1d9d64fc5f/pkg/pki/pki.go#L57-L113)|
