...
**Version:** 1.10  
**Status:** Amended: EH-F-01 reworded — the health-check endpoint changes verb from `POST /api/health` to `GET /api/health`, since a health check is safe and idempotent (standard REST/probe convention). EH-F-02 reworded — the registration endpoint's path changes from `POST /api/register` to `POST /api/users`, a resource-oriented name for the resource it creates; CL-F-02, CL-F-09 and UC-01 updated to match. EH-F-03 (`POST /api/login`) deliberately left unchanged: see the new "Deliberate design choices" bullet in section 1.2 — there is no addressable session resource in this system (no session ID is returned, nothing can be inspected or revoked; RU-09/RISK-01 already puts revocation out of scope), so renaming it to a resource-style path (e.g. `/api/sessions`) would be REST-shaped naming without REST-shaped behavior behind it. Code permalinks for EH-F-01/EH-F-02 will need re-pinning to the commit that lands this change, per the usual periodic audit (see the 1.9 entry above).
